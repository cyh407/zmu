  ...
  <table class="table table-bordered table-hover">
    ...
    <tbody>
      <?php if(is_array($leave)): foreach($leave as $key=>$v): ?><tr>
          ...
          <td>
          <?php if($v['status'] == 0): ?><a class="btn btn-sm btn-success" href="<?php echo U('Admin/Leave/accept',array('id'=>$v['id']));?>">同意</a>
            <a class="btn btn-sm btn-danger" href="<?php echo U('Admin/Leave/no',array('id'=>$v['id']));?>">拒绝</a>
            <?php elseif($v['status'] == 1): ?>同意
            <?php else: ?>不同意<?php endif; ?>
          </td>
